; BMC d=8 for DmaEngineSmall -- expects UNSAT
...
(declare-const |state_5| |DMA_s|)
(declare-const |state_6| |DMA_s|)
(declare-const |state_7| |DMA_s|)
(declare-const |state_8| |DMA_s|)

(assert (|DMA_t| |state_0| |state_1|))
(assert (|DMA_t| |state_1| |state_2|))
(assert (|DMA_t| |state_2| |state_3|))
(assert (|DMA_t| |state_3| |state_4|))
(assert (|DMA_t| |state_4| |state_5|))
(assert (|DMA_t| |state_5| |state_6|))
(assert (|DMA_t| |state_6| |state_7|))
(assert (|DMA_t| |state_7| |state_8|))

; Negate: write to ch0 and read same ch0 but rd_data != wr_data
(assert (|DMA#wr_en|  |state_7|))
(assert (= (|DMA#wr_ch|  |state_7|) #b00))
(assert (= (|DMA#rd_ch|  |state_7|) #b00))
(assert (not (= (|DMA#rd_data| |state_8|) (|DMA#wr_data| |state_7|))))
