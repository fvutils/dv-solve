; BMC d=1 for RegfileAddrAlias -- expects UNSAT
...
(assert (|RegfileAlias#wr_en|   |state_0|))
(assert (not (= (|RegfileAlias#rd_addr| |state_0|)
               (|RegfileAlias#wr_addr| |state_0|))))
; rd_data in next state should equal old mem[rd_addr]; negate:
(assert (not (= (|RegfileAlias#rd_data| |state_1|)
               (select (|RegfileAlias#mem| |state_0|)
                       (|RegfileAlias#rd_addr| |state_0|)))))
